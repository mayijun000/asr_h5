			...
			<div class="list_cont">
				<div class="form">
					...
					<p class="other">
						...
						<input type="text" placeholder="请输入联系人" v-model="applyPersonType.name" @click="showPersonPicker">
						<i class="right">&nbsp;</i>
						<mt-popup v-model="person" position="bottom" class="region-popup">  
							<div class="picker-toolbar">
								<span class="mint-datetime-action mint-datetime-cancel" @click="closePersonPicker">取消</span> 
								<span class="mint-datetime-action mint-datetime-confirm" @click="okPersonPicker">确定</span>
							</div>
        					<mt-picker :slots="personListSlots" valueKey="optionText" :visibleItemCount ="3" @change="onPersonChange" :itemHeight="50"></mt-picker>  
	      				</mt-popup>
					</p>
					<p>
...
<script>
	...
	import { sendMobileCode, checkMobileCode, checkRealName, submitApplyStepone, applyPerson} from "@/api/getData"
	...
	export default {
		...
		methods: {
			...
			async getApplyPerson(){
				try {
					const res = await applyPerson();
					if(res.errorCode == "0"){
						let applyPerson = res.dataBody.applyPerson;
						this.personListSlots[0].values = applyPerson;
					}else {
						Toast({
							message: err.toString(),
						})
		    ...
		    onPersonChange(picker, values){
		    	if(this.personInit){
		    		this.applyPersonType.name = values[0].optionText;
		    		this.applyFrom.loanType = values[0].value;
		    	}else{
		    		this.personInit = true;
		    	}
