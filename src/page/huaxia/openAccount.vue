	...
	<div>
		...
		<div class="list" v-show="isShow == 1">
			<div class="list_conts">
				<h3>填写信息</h3>			
				<p class="bank_tip">开立华夏银行二类账户申请</p>
			</div>
				...
				<div class="list_cont">
					<div class="form">
						...
						<p>
							...
							<input type="text" placeholder="请输入所属开户行" required='不能为空' :disabled="inputPageOne.openBankName?true:false" v-model="inputPageOne.openBankName">
						</p>
						<p class='bot_none'>
							<label><span>*</span>上传身份证照片:</label>
						</p>						
						<p class='bot_none img_box'>
							<img src="../../static/sfz.png" ref="img1" class="apply_btn sf_1" @click="actionSheet('file1')">
							<img src="../../static/sfz2.png" ref="img2" class="apply_btn sf_2" @click="actionSheet('file2')"/>							
						</p>
						<p class='bot_none het_none'>
							<input type="file" ref="file1" accept="image/jpeg,image/x-png,image/gif" @change="uploading($event)">
							<input type="file" ref="file2" accept="image/jpeg,image/x-png,image/gif" @change="uploadings($event)">
						</p>
						<p>	
...
<script>
	...
	import { getAllApplyByApply, sendHxhzMobileVerifyCode, establishAccount } from "@/api/getData"
	...
	export default {
		...
		methods: {
			...
			async sendHxhzMobileVerifyCode() {
				...
				if(mobile == "") {
					...
					try {
						...
						if(res.errorCode == "0") {
							let smsCodeid = res.dataBody.smsCodeId;
			...
			async Account() {
				...
				try {
					const res = await establishAccount(formData);
					if(res.errorCode == "0") {
						this.isShow = 2;
						let openBank = res.dataBody.secondeBankCardNo;
						let openBankYz = openBankNo.substring(0,6)+" **** **** "+openBankNo.substring(openBankNo.length-4,openBankNo.length);
						this.bankCode = openBank;
						this.bankCodeYz = openBankYz;
					} else {
						throw new Error(res.errorString);
					}
				} catch(err) {
		    ...
		    uploading(event){
		   	...
		   	uploadings(event){
		       	...
		       	var windowURL = window.URL || window.webkitURL;	
		        //创建图片文件的url		
		        this.$refs.img2.src = windowURL.createObjectURL(event.target.files[0]);
		        this.file.push(file)
		   	},
		    getCamera(){  
		      	//console.log("打开照相机")  
