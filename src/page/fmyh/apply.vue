...
<script>
	...
	import { sendMobileCode, checkMobileCode, verifyBusinessInfo, submitApplyStepone } from "@/api/getData"
	...
	export default {
		...
		methods: {
			...
			//工商验证
			async verifyBusinessInfo() {
				try {
					const res = await verifyBusinessInfo({
					...
					if(res.errorCode == "0") {
						var qyfrCheck = res.dataBody.isFddbrCheck;
						if(qyfrCheck) {
