		...
		<div class="list">
			...
			<div class="list_cont">
				<div class="form">
					...
					<p>
						<label><span>*</span>借款人手机号码:</label>
						<input type="text" placeholder="请输入手机号码" maxlength="11"  v-model="applyFrom.contactMobile" />
					...
					<p>
						<label><span>*</span>申请贷款额度:</label>
						<input type="text" placeholder="请输入申请贷款额度" style="width:35%;" v-model="applyFrom.loanAmount">
						<i class="cancel unit">万元</i>
					</p>
					<p>
						<label><span>*</span>申请贷款期限:</label>
						<input type="text" placeholder="请输入申请贷款期限" style="width:35%;" v-model="applyFrom.loanTerm">
						<i class="cancel unit">月</i>
					</p>
					<!-- <p>
					...
					<p class="other">
						<label for=""><span>*</span>独资且未婚:</label>
						<input type="text" placeholder="请选择是否独资且未婚" required='不能为空' v-model="applyFrom.ownedName" @click="showOwnedPicker" readonly="readonly">
						<i class="right">&nbsp;</i>
						<mt-popup v-model="typeOwned" position="bottom" class="region-popup">  
								<div class="picker-toolbar">
									<span class="mint-datetime-action mint-datetime-cancel" @click="closeOwnedPicker">取消</span> 
									<span class="mint-datetime-action mint-datetime-confirm" @click="okOwnedPicker">确定</span>
								</div>
	        					<mt-picker :slots="ownedListSlots" valueKey="name" :visibleItemCount ="3" @change="onOwnedChange" :itemHeight="50"></mt-picker>  
	      				</mt-popup>
					</p>
					<p class="other" v-if="applyFrom.owned==0">
						<label for=""><span>*</span>本人为实际控制人:</label>
						<input type="text" placeholder="请选择是否本人为实际控制人" required='不能为空' v-model="applyFrom.actualName" @click="showActualPicker" readonly="readonly">
						<i class="right">&nbsp;</i>
						<mt-popup v-model="typeActual" position="bottom" class="region-popup">  
								<div class="picker-toolbar">
									<span class="mint-datetime-action mint-datetime-cancel" @click="closeActualPicker">取消</span> 
									<span class="mint-datetime-action mint-datetime-confirm" @click="okActualPicker">确定</span>
								</div>
	        					<mt-picker :slots="actualListSlots" valueKey="name" :visibleItemCount ="3" @change="onActualChange" :itemHeight="50"></mt-picker>  
	      				</mt-popup>
					</p>
					<p class="other" v-if="applyFrom.owned==0">
						<label for=""><span>*</span>共同借款人:</label>
						<input type="text" placeholder="请选择共同借款人" required='不能为空' v-model="applyFrom.borrowerType" @click="showBorrowerPicker" readonly="readonly">
						<i class="right">&nbsp;</i>
						<mt-popup v-model="typeBorrower" position="bottom" class="region-popup">  
								<div class="picker-toolbar">
									<span class="mint-datetime-action mint-datetime-cancel" @click="closeBorrowerPicker">取消</span> 
									<span class="mint-datetime-action mint-datetime-confirm" @click="okBorrowerPicker">确定</span>
								</div>
	        					<mt-picker :slots="borrowerListSlots" valueKey="name" :visibleItemCount ="3" @change="onBorrowerChange" :itemHeight="50"></mt-picker>  
	      				</mt-popup>
					</p>
					<p v-if="applyFrom.owned==0">
						<label for=""><span>*</span>共同借款人姓名:</label>
						<input type="text" placeholder="请填写共同借款人姓名" required='不能为空' v-model="applyFrom.borrowerName">
					</p>
					<p v-if="applyFrom.owned==0">
						<label for=""><span>*</span>共同借款人手机号:</label>
						<input type="text" placeholder="请填写共同借款人手机号" required='不能为空' v-model="applyFrom.borrowerPhone">
					</p>
					<p>
...
<script>
	...
	import { sendMobileCode, checkMobileCode, checkRealName, submitApplyStepone_lxxd, getGsxx, getBankCardList} from "@/api/getData";
	...
	export default {
		...
		methods: {
			...
			async getGsxx() {
				try {
					...
					if(res.errorCode == "0") {
						...
						this.ownedListSlots[0].values=[{name: "是",id: "1"	},{name: "不是",id: "0"	}];
						this.actualListSlots[0].values=[{name: "是",id: "1"	},{name: "不是",id: "0"	}];
					}
				} catch(err) {
					console.log('错误', err);
					Toast({
			...
			async submitApplyInfo() {
				let data = {};
				let dataNx = {};
				dataNx.loanType = "fddbr",
				dataNx.idtfno = this.applyFrom.idtfno;
				dataNx.openBankNo = this.applyFrom.cardNo;
				dataNx.openBankName = this.applyFrom.cardBankName;
				dataNx.ownedFlag = this.applyFrom.owned;
				//独资且未婚   是
				if(this.applyFrom.owned == "0"){
					dataNx.acFlag =this.applyFrom.actual;
					dataNx.cbType = this.applyFrom.borrower;
					dataNx.cbName = this.applyFrom.borrowerName;
					dataNx.cbMobile = this.applyFrom.borrowerPhone;
				}
				...
				try {
					const res = await submitApplyStepone_lxxd(data);
					if(res.errorCode == "0") {
						Indicator.close();
			...
			checkData() {
				...
				// 	return false;
					...
					return false;
				}else if(applyFrom.owned==0&&applyFrom.borrowerName == "") {
					Toast({
						message: '请输入共同借款人姓名',
					});
					return false;
				}else if(applyFrom.owned==0&&applyFrom.borrowerPhone == "") {
					Toast({
						message: '请输入共同借款人手机号',
					});
					return false;
			...
			submitApply() {
				if(this.checkBox) {
					if(this.checkData()) {
						// this.checkMobileCodeData(); //校验短信验证码
						this.checkRealNameData();
					}
				} else {
					Toast({
			...
			closeOwnedPicker(){  
		        this.typeOwned = false;  
		    }, 
		    okOwnedPicker(type){
		    	this.closeOwnedPicker();
		    },
		    onOwnedChange(picker, values){
		    	if(this.ownedInit){
		    		this.applyFrom.ownedName = values[0].name;
		    		this.applyFrom.owned = values[0].id;
		    	}else{
		    		this.ownedInit = true;
		    	}
			...
			showActualPicker(){
				this.typeActual = true;  
			},
			closeActualPicker(){  
		        this.typeActual = false;  
		    }, 
		    okActualPicker(type){
		    	this.closeActualPicker();
		    },
		    onActualChange(picker, values){
		    	if(this.actualInit){
		    		this.applyFrom.actualName = values[0].name;
					this.applyFrom.actual = values[0].id;
					if(values[0].id==0){
						this.borrowerListSlots[0].values=[{name: "股东",id: "3"	}];
					}else{
						this.borrowerListSlots[0].values=[{name: "共同借款人",id: "1"	},{name: "配偶",id: "2"	}];
					}
		    	}else{
		    		this.actualInit = true;
		    	}
		    	
		    },
			showBorrowerPicker(){
				this.typeBorrower = true;  
			},
			closeBorrowerPicker(){  
		        this.typeBorrower = false;  
		    }, 
		    okBorrowerPicker(type){
		    	this.closeBorrowerPicker();
		    },
		    onBorrowerChange(picker, values){
		    	if(this.borrowerInit){
		    		this.applyFrom.borrowerType = values[0].name;
		    		this.applyFrom.borrower = values[0].id;
		    	}else{
		    		this.borrowerInit = true;
		    	}
